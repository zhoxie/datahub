...
public class DatasoureCategories extends BaseEntityResource<
  ...
  public Task<DatasourceCategory> get(
    ...
    final Set<String> projectedAspects = aspectNames == null ? Collections.emptySet() : new HashSet<>(
        Arrays.asList(aspectNames).stream().map(fullName -> {
                  RecordDataSchema aspectSchema = RecordUtils.toRecordTemplate(fullName, new DataMap()).schema();
                  Object aspectAnnotationObj = aspectSchema.getProperties().get(AspectAnnotation.ANNOTATION_NAME);
